...
def log(msg: Any = "", level: str = "INFO"):
    ...
    if _LOGGER is None:
        ...
        return
    # Inject level as part of the line (FileLogger also prefixes)
...
def _strip_code_fences(s: str) -> str:
    # If fenced code block exists, prefer first fenced content
...
def _extract_balanced_json_object(s: str) -> Optional[str]:
    # Extract first balanced top-level JSON object {...} respecting string quotes
    start = s.find("{")
    if start == -1:
        return None
    i = start
...
def _cleanup_relaxed_json_text(s: str) -> str:
    # Minimal cleanup: remove trailing commas before closing } or ]
    s = re.sub(r",\s*(\})", r"\1", s)
    s = re.sub(r",\s*(\])", r"\1", s)
    return s
        ...
        try:
            ...
            if JSON_PARSER_STRICTNESS != "strict":
                try:
                    ...
                    pass
    # 4) crude range from first { to last }
        ...
        try:
            ...
            if JSON_PARSER_STRICTNESS != "strict":
                try:
                    ...
                    pass
    # 5) last attempt on raw_text if candidate was different
...
def _ensure_required_fields(obj: Dict[str, Any], schema: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    if sch_type == "object" and isinstance(props, dict):
        # Initialize missing required with defaults
        for k in required:
            if k not in obj:
                prop = props.get(k, {})
                d = _default_for_type(prop.get("type"))
                obj[k] = d
        # Optionally coerce simple nested types
...
def agentic_multi(query_original: str, logger: Optional[FileLogger] = None, log_context_prefix: Optional[str] = None) -> Dict[str, Any]:
    ...
    Optionally sets a per-thread log prefix (e.g., sg=<id>) for log correlation.
    """
    global _LOGGER
    prev_logger = _LOGGER
...
if __name__ == "__main__":
    try:
        ...
        if not user_query:
            ...
            # Run the new agentic supervisor by default
