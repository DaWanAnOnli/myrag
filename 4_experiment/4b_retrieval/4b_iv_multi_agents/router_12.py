#!/usr/bin/env python3
# multi_agent.py
# Self-contained multi-agent RAG with a Router Agent:
# - Router Agent chooses ONE pipeline per question: GraphRAG or NaiveRAG
...
#
# Note:
# - No imports from existing scripts; this file is self-contained.
...
# Dataset folder for original chunk pickles (same convention as existing code)
...
AJ_ANSWER_MAX_CHARS = int(os.getenv("AJ_ANSWER_MAX_CHARS", "20000000"))  # truncate in judge prompts
...
# Router parameters
ROUTER_MAX_TOKENS = int(os.getenv("ROUTER_MAX_TOKENS", "4096"))
...
def router_agent_decide_pipeline(user_query: str, lang: str) -> Dict[str, Any]:
    prompt = f"""
...
"""
    ...
    ent_count = len(re.findall(r"\b([A-Z][a-zA-Z0-9_]+)\b", (user_query or "")))
...
def run_multi_agent(user_query: str) -> Dict[str, Any]:
    """
    Router-based orchestrator:
    - Uses Router Agent to choose one pipeline: GraphRAG or NaiveRAG.
    - Runs only the chosen pipeline.
    - Returns final answer and diagnostics.
    For compatibility with existing runners, the 'aggregator' field carries the router decision.
    """
    global _LOGGER
    ...
    try:
        ...
        # Router decides which pipeline to use
        router = router_agent_decide_pipeline(user_query, user_lang)
        route = router.get("decision","graphrag")
        route_rationale = router.get("rationale","")

        graphrag_out: Dict[str, Any] = {}
        naiverag_out: Dict[str, Any] = {}
        final_answer = ""

        if route == "graphrag":
            log("[Router] Executing GraphRAG pipeline.")
            chunk_store = ChunkStore(LANGCHAIN_DIR, set(SKIP_FILES))
            graphrag_out = graph_rag_run(user_query, chunk_store=chunk_store, user_lang=user_lang)
            final_answer = graphrag_out.get("final_answer","") or ""
        else:
            log("[Router] Executing NaiveRAG pipeline.")
            naiverag_out = naive_rag_run(user_query, user_lang=user_lang)
            final_answer = naiverag_out.get("final_answer","") or ""

        total_ms = (time.time()-t_all)*1000
        log("\n=== Multi-Agent RAG (Router) summary ===")
        log(f"- Router decision: {route} | rationale: {route_rationale}")
        log(f"- GraphRAG iterations: {graphrag_out.get('iterations_used', 0)}")
        log(f"- NaiveRAG iterations: {naiverag_out.get('iterations_used', 0)}")
        log(f"- Total runtime: {total_ms:.0f} ms")
        log("\n=== Final Answer ===")
        log(final_answer)
        log(f"\nLogs saved to: {log_file}")

        # For compatibility with run_multi_agent.py, expose 'aggregator' decision as router_*.
        compat_aggregator = {"decision": f"router_{route}"}

        return {
            "final_answer": final_answer,
            "router": {"decision": route, "rationale": route_rationale, "fallback": bool(router.get("fallback", False))},
            "aggregator": compat_aggregator,  # compatibility shim for existing runner
...
if __name__ == "__main__":
    try:
        ...
        if not user_query:
            ...
            # Print only the final answer for CLI UX
            print("\n" + "="*100)
            print("Router decision:", (result.get("router") or {}).get("decision"))
            print("Final Answer:")
