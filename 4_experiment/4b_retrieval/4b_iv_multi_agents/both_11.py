...
- Comprehensive logging similar to existing scripts; per-iteration tagging; QPS controls; retries.

Note:
- This file is self-contained: it does not import anything from your other scripts.
- It expects a Neo4j database populated with:
  * A 'triple_vec' vector index on (:Triple {embedding})
  * Entity vector indexes: 'document_vec', 'content_vec', 'expression_vec' (optional if entity-centric disabled)
  * A 'chunk_embedding_index' vector index on (:TextChunk {embedding}) for naive retrieval
- It expects a dataset directory of pickled chunks for ChunkStore (GraphRAG path). Configure via LANGCHAIN_DIR.
...
def _rand_wait_seconds(low=50.0, high=80.0) -> float:
    ...
    try:
        ...
        def run_G():
            try:
                ...
                G_out = run_graphrag_loop(query_original, chunk_store, user_lang)
                G_res.update(G_out)
            except Exception as e:
                exc["G"] = str(e)
        ...
        def run_N():
            try:
                ...
                N_out = run_naiverag_loop(query_original, user_lang)
                N_res.update(N_out)
            except Exception as e:
                exc["N"] = str(e)
        ...
        if "G" in exc and not g_answer and n_answer:
            ...
            g_meta = {"iterations_used": G_res.get("iterations_used"), "context_hint": "Graph triples + chunks", "per_iteration_count": len(G_res.get("per_iteration", []) or [])}
            n_meta = {"iterations_used": N_res.get("iterations_used"), "context_hint": "Top chunk content", "iteration_runs_count": len(N_res.get("iteration_runs", []) or [])}
