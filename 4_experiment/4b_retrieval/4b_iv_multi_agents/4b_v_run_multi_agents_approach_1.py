...
"""
run-multi-agent.py
Batch runner for multi-agent RAG (GraphRAG + NaiveRAG + Aggregator).
...
- Each worker overrides GOOGLE_API_KEY (env) before importing multi_agent.py (which reads env on import).
- Per-question logs are tee'd to files under question_terminal_logs_multi_agent.
- Part files from each worker are merged into one JSONL in the configured output directory.
...
Output JSONL (combined, one line per input question):
  {
    "id": <int>,
    "question": <str>,
    "ground_truth": <str>,
...
MAX_QUESTIONS = int(os.getenv("MAX_QUESTIONS", "0"))  # process all by default
...
def worker_main(worker_id: int,
    ...
    # Configure key for this process (module reads env on import)
    if key_value:
        ...
        for i, task in enumerate(tasks, 1):
            ...
            with log_file.open("w", encoding="utf-8") as lf:
                ...
                with contextlib.redirect_stdout(tee_out), contextlib.redirect_stderr(tee_err):
                    ...
                    error_msg = None
                    try:
                        result = ma.run_multi_agent(question)
                        final_answer = (result or {}).get("final_answer", "")
                        if not isinstance(final_answer, str):
                            final_answer = json.dumps(final_answer, ensure_ascii=False)

                        graphrag_answer = ((result or {}).get("graphrag") or {}).get("final_answer", "")
                        naiverag_answer = ((result or {}).get("naiverag") or {}).get("final_answer", "")
                        decision = ((result or {}).get("aggregator") or {}).get("decision", "")
                    except Exception as e:
                        error_msg = str(e)
                        final_answer = f"(error during run_multi_agent: {error_msg})"
    ...
    # Best-effort cleanup of Neo4j driver inside module
    try:
        if hasattr(ma, "driver") and ma.driver is not None:
            ma.driver.close()
    except Exception:
        pass
...
def main():
    ...
    log("Worker assignment (by batches):")
