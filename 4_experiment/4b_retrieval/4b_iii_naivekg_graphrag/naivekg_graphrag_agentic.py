...
DEFAULT_LANGCHAIN_DIR = (Path(__file__).resolve().parent / "../../../dataset/samples/3_indexing/3a_langchain_results/").resolve()
...
def search_similar_entities_by_embedding(q_emb: List[float], k: int) -> List[Dict[str, Any]]:
    """
    Search the simple ER KG's entity vector index ('entity_vec') and return top-k entities.
    Nodes are labeled :Entity and carry properties: key, name, type='Entity', embedding.
    """
    candidates: List[Dict[str, Any]] = []
    try:
        # Single unified entity index in the simplified KG
        candidates.extend(_vector_query_nodes("entity_vec", q_emb, k))
    except Exception as e:
        log(f"[Warn] entity_vec query failed: {e}")

    # Deduplicate by (key, type) while keeping the best score (harmless even if single source)
...
def expand_from_entities(entity_keys: List[str], hops: int, per_hop_limit: int) -> List[Dict[str, Any]]:
    """
    Traverse from seed :Entity nodes via any relationship that has r.triple_uid,
    fetch the corresponding :Triple node and its endpoints (:Entity).
    """
