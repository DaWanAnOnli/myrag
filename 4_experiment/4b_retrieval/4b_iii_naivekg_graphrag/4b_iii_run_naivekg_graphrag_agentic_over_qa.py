#!/usr/bin/env python3
"""
Parallel runner for naivekg_graphrag_agentic over QA pairs.

What it does:
- Reads underscore-path QA pairs JSONL with structure:
    { "id": <int>, "source_filename": <str>, "group_index": <int>,
      "page_start": <int>, "page_end": <int>, "question": <str>,
      "answer": <str>, "verification_choice": <str>, ... }
- Does NOT filter by verification_choice (assumes upstream already filtered).
- Spawns one worker per GOOGLE_API_KEY_N found in ../../../.env, distributes tasks evenly.
- Each worker sets its own GOOGLE_API_KEY (env var + module constant) for naivekg_graphrag_agentic.py.
- Captures per-question logs into question_terminal_logs.
- Warns if output directory is not empty; requires confirmation and wipes it before writing.
- Writes a single combined JSONL with only:
    { "id", "question", "ground_truth", "agentic_naivekg_graphrag_answer" }
"""
...
def worker_main(worker_id: int,
    ...
    try:
        import naivekg_graphrag_agentic as rag
    except Exception as e:
        log(f"[Worker {worker_id}] ERROR: Could not import naivekg_graphrag_agentic.py: {e}")
    ...
    with part_path.open("w", encoding="utf-8") as out_f:
        for i, task in enumerate(tasks, 1):
            ...
            with log_file.open("w", encoding="utf-8") as lf:
                ...
                with contextlib.redirect_stdout(tee_out), contextlib.redirect_stderr(tee_err):
                    ...
                    try:
                        ...
                        naivekg_answer = (result or {}).get("final_answer", "")
                        if not isinstance(naivekg_answer, str):
                            naivekg_answer = json.dumps(naivekg_answer, ensure_ascii=False)
                    except Exception as e:
                        error_msg = str(e)
                        naivekg_answer = f"(error during agentic_graph_rag: {error_msg})"
            ...
            # Stream JSONL output (one line per question)
            out_record = {
                "id": qid,
                "question": question,
                "ground_truth": ground_truth,
                "agentic_naivekg_graphrag_answer": naivekg_answer
            }
            out_f.write(json.dumps(out_record, ensure_ascii=False) + "\n")
            out_f.flush()
            processed += 1

    # Best-effort cleanup if module exposes a driver
    try:
...
def main():
    ...
    # Warn if not empty; confirm and wipe
    ensure_output_dir_ready(output_dir)
    logs_dir.mkdir(parents=True, exist_ok=True)

    # Load all QA pairs (already filtered upstream; we do not filter here)
    ...
    log(f"Processing {len(selected)} out of {total_records} records ({limit_info}).")

    # Prepare output file name
    ts = datetime.now().strftime("%Y%m%d-%H%M%S")
    out_path = output_dir / f"naivekg_graphrag_answers_{ts}.jsonl"

    # Build tasks with stable global indices and minimal fields
    tasks = []
    for global_idx, rec in enumerate(selected, 1):
        ...
        tasks.append({
            ...
            "id": rec.get("id", -1),
            "question": question,
            "ground_truth": rec.get("answer") or rec.get("ground_truth") or ""
        })

    total_tasks = len(tasks)
    if total_tasks == 0:
        log("No non-empty questions after selection. Exiting.")
        return
