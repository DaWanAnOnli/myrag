...
ENTITY_SUBGRAPH_HOPS = 3               # hop-depth for subgraph expansion from matched entities
...
def query_triple_to_text(t: Dict[str, Any]) -> str:
    # Align with KG triple embedding style: "subject predicate object" (no brackets)
...
def run_cypher_with_retry(cypher: str, params: Dict[str, Any]) -> List[Any]:
    """
    Execute a Cypher query with unlimited retries on error.
    Wait 5–20s between attempts.
...
def build_context_from_triples(triples_ranked: List[Dict[str, Any]]) -> Tuple[str, str]:
    """
    Build context and summary using only triples and their quotes.
    Returns (context_text, summary_text).
    """
    lines = []
    lines.append("Ringkasan triple yang relevan (dengan kutipan bukti):")
    for t in triples_ranked[:min(50, len(triples_ranked))]:
        s = t.get("subject") or ""
        p = t.get("predicate") or ""
        o = t.get("object") or ""
        quote = (t.get("evidence_quote") or "").strip()
        if len(quote) > 300:
            quote = quote[:300] + "..."
        lines.append(f"- {s} {p} {o} | bukti: {quote}")
    summary_text = "\n".join(lines)
    context_text = summary_text  # context is just the triples + quotes
...
Context summary (triples + quotes):
\"\"\"{context_summary}\"\"\"
...
def agentic_graph_rag(query_original: str) -> Dict[str, Any]:
    ...
    try:
        ...
        for it in range(1, MAX_ITERS + 1):
            ...
            # Step 0: Embed whole user query (for fallback scoring)
            t0 = now_ms()
            ...
            # Step 5: Rerank triples by mean similarity to query triples (fallback to query embedding)
            t5 = now_ms()
            ...
            t_rerank = dur_ms(t5)
            log(f"[Step 5] Triple rerank done in {t_rerank:.0f} ms; selected {len(triples_ranked)}")

            # Build context only from triples + quotes
            context_text, context_summary = build_context_from_triples(triples_ranked)
            log("\n[Context summary for this iteration]:")
            log(context_summary)

            # Step 6: Agent 2 – Answer
            t6 = now_ms()
