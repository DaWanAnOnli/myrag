#!/usr/bin/env python3
# agentic_rag.py
# Sequential Agentic RAG with Intermediate Question Generator (IQG) and Query Modifier
#
# Pipeline:
# - Detect user language
# - Intermediate Question Generator (IQG) plans an ordered list of dependent Intermediate Questions (IQs)
#   (falls back to a single IQ identical to the original query if sequential decomposition isn’t needed)
# - For each IQ in order:
#     * Query Modifier uses all previous IQ/Answer pairs + the planned next IQ to produce a completed/enriched IQ
#     * Run the existing RAG loop (Agent 2 Answerer + Agent 3 Judge) on the completed IQ
#     * Store IQs and answers to inform the following step
# - After the last IQ is answered, that answer is the final answer (no aggregator)

import os, time, json, pickle, re, random, threading
from pathlib import Path
from typing import Any, Dict, List, Tuple, Optional
from concurrent.futures import ThreadPoolExecutor, as_completed  # (not used for IQ steps; kept for potential future use)
...
# Agent loop (per-IQ, inner loop)
ANSWER_MAX_TOKENS = int(os.getenv("ANSWER_MAX_TOKENS", "4096"))
MAX_ITERS = int(os.getenv("MAX_ITERS", "3"))

# Intermediate Questions (hardcoded cap, as requested)
MAX_IQ = 6  # maximum number of intermediate questions allowed (hard limit)

# Optional clamps for IQ utilities
IQ_COMPLETED_CLAMP = 3000  # max chars for a completed/enriched IQ
IQ_HISTORY_ANSWER_CLAMP = 6000  # clamp each previous answer passed to Query Modifier
...
# ----------------- Intermediate Question Generator (IQG) -----------------
IQ_PLAN_SCHEMA = {
  "type": "object",
  "properties": {
...
def generate_intermediate_questions(query_original: str, user_lang: str) -> List[Dict[str, Any]]:
    prompt = f"""
You are the Intermediate Question Generator (IQG).
Goal: Create an ordered sequence of dependent intermediate questions (IQs) needed to answer the user's original question.
- If sequential decomposition is NOT necessary, return exactly 1 IQ equal to the original question.
- IQs may depend on prior IQ answers; keep them minimal, focused, and non-overlapping.
...
Return JSON with key "intermediate_questions" as a list of objects:
  - question: string (the intermediate question)
  - reason: brief string (why this step is needed; optional)
"""
    est = estimate_tokens_for_text(prompt)
    log("\n[IQ Plan] Generator Prompt:")
    log(prompt)
    log(f"[IQ Plan] Prompt size: {len(prompt)} chars, est_tokens≈{est}")
    out = safe_generate_json(prompt, IQ_PLAN_SCHEMA, temp=0.2) or {}
    raw = out.get("intermediate_questions", []) or []

    # Normalize, deduplicate, clamp, cap
    seen = set()
    plan: List[Dict[str, Any]] = []
    for item in raw:
        q = clamp((item.get("question") or "").strip(), 2000)
        if not q:
            continue
        ...
        if key in seen:
            continue
        seen.add(key)
        plan.append({"question": q, "reason": clamp((item.get("reason") or "").strip(), 500)})
        if len(plan) >= MAX_IQ:
            break

    if not plan:
        plan = [{"question": query_original, "reason": "Fallback: no decomposition required or planning failed."}]

    # Attach step indices
    for i, step in enumerate(plan, 1):
        step["id"] = i

    log("[IQ Plan] Planned Intermediate Questions:")
    log(json.dumps(plan, ensure_ascii=False, indent=2))
    return plan

# ----------------- Query Modifier (completes/enriches next IQ) -----------------
QUERY_MODIFIER_SCHEMA = {
  ...
  "required": ["completed_question"]
}

def modify_next_intermediate_question(prev_qas: List[Dict[str, str]],
                                      planned_next_question: str,
                                      original_query: str,
                                      user_lang: str) -> Dict[str, Any]:
    """
    Use previous IQ/Answer pairs to complete/enrich the planned next IQ.
    Returns {completed_question, notes}
    """
    # Build a concise history string
    hist_lines = []
    for i, qa in enumerate(prev_qas, 1):
        qi = clamp(qa.get("question", ""), 1500)
        ai = clamp(qa.get("answer", ""), IQ_HISTORY_ANSWER_CLAMP)
        hist_lines.append(f"- Step {i} Q: {qi}\n  A: {ai}")
    history_text = "\n".join(hist_lines) if hist_lines else "(none)"

    prompt = f"""
You are the Query Modifier.
Task: Given the original question, the already-answered previous intermediate questions (with their answers),
and the next planned intermediate question, produce a completed/enriched version of the next question.

Rules:
- Use previous answers only to specialize/instantiate the next question (e.g., fill in computed values, narrow scope).
- Do NOT introduce new facts or assumptions not implied by previous answers or the planned IQ.
- Keep the completed question self-contained and in the user's language.
- Return JSON with "completed_question" and optional "notes".

Original user question:
\"\"\"{original_query}\"\"\"

Previously answered IQs (Q/A):
\"\"\"{history_text}\"\"\"
...
"""
    ...
    out = safe_generate_json(prompt, QUERY_MODIFIER_SCHEMA, temp=0.1) or {}
    completed = clamp((out.get("completed_question") or "").strip(), IQ_COMPLETED_CLAMP)
    notes = clamp((out.get("notes") or "").strip(), 1000)

    if not completed:
        # Fallback to planned question if modifier returns nothing
        completed = clamp(planned_next_question, IQ_COMPLETED_CLAMP)
        notes = "Fallback: modifier produced empty completed_question; using the planned IQ as-is."

    log("[Query Modifier] Completed/Enriched IQ:")
    log(json.dumps({"completed_question": completed, "notes": notes}, ensure_ascii=False, indent=2))
    return {"completed_question": completed, "notes": notes}

# ----------------- Single-query Agentic loop (reused per IQ) -----------------
def run_agentic_loop_for_query(query_original: str, user_lang: str, log_prefix: str = "") -> Dict[str, Any]:
    """
    Executes the existing Answerer+Judge iterative loop for a single query (used per intermediate question).
    ...
    for it in range(1, MAX_ITERS + 1):
        ...
        # Agent 1 & 1b (optional signals)
        t1 = time.time()
        _ = agent1_extract_entities_predicates(query_for_iter)
...
# ----------------- Orchestrator: Sequential IQ pipeline -----------------
def agentic_rag(query_original: str) -> Dict[str, Any]:
    ...
    try:
        log("=== Agentic RAG (Sequential IQ) run started ===")
        ...
        log(f"- MAX_IQ (hardcoded)={MAX_IQ}, IQ_COMPLETED_CLAMP={IQ_COMPLETED_CLAMP}, IQ_HISTORY_ANSWER_CLAMP={IQ_HISTORY_ANSWER_CLAMP}")

        user_lang = detect_user_language(query_original)
        log(f"[Language] Detected user language: {user_lang}")

        # Step A: Generate the IQ plan
        iq_plan = generate_intermediate_questions(query_original, user_lang)

        # Step B: Execute IQs sequentially
        log("\n=== Executing Intermediate Questions sequentially ===")
        iq_results: List[Dict[str, Any]] = []
        previous_qas: List[Dict[str, str]] = []  # history for Query Modifier

        for step in iq_plan:
            sid = int(step.get("id", len(iq_results) + 1))
            planned_q = step.get("question", "").strip()
            reason = (step.get("reason") or "").strip()

            log_with_prefix(f"[IQ {sid}]", f"Planned question: {planned_q}")
            if reason:
                log_with_prefix(f"[IQ {sid}]", f"Reason: {reason}")

            # Query Modifier to complete/enrich the planned IQ using previous answers
            qm_out = modify_next_intermediate_question(previous_qas, planned_q, query_original, user_lang)
            completed_q = qm_out.get("completed_question") or planned_q
            modifier_notes = qm_out.get("notes", "")

            # Run the existing RAG loop on the completed question
            result = run_agentic_loop_for_query(completed_q, user_lang=user_lang, log_prefix=f"[IQ {sid}]")

            # Record this step
            step_record = {
                "step_index": sid,
                "planned_question": planned_q,
                "completed_question": completed_q,
            ...
            iq_results.append(step_record)

            # Append to history for the next Query Modifier iteration
            previous_qas.append({"question": completed_q, "answer": result["final_answer"]})

        # Step C: Final answer = answer of the last IQ
        final_answer = iq_results[-1]["answer"] if iq_results else "(No answer produced.)"

        # Summary
        sufficient_count = sum(1 for r in iq_results if r.get("judge_sufficient"))
        total_iters = sum(r.get("iterations", 0) for r in iq_results)

        log("\n=== Sequential IQ RAG summary ===")
        log(f"- IQs planned: {len(iq_plan)}")
        log(f"- IQs executed: {len(iq_results)} (judge sufficient={sufficient_count})")
        log(f"- Total iterations across IQs: {total_iters}")
