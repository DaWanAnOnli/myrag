#!/usr/bin/env python3
# agentic_rag.py
# Naive Agentic RAG (Answerer-only):
# - Embed the query
...
def build_context_from_chunks(chunks: List[Dict[str, Any]], max_chunks: int) -> str:
    ...
    for i, c in enumerate(chosen, 1):
        ...
        lines.append(f"[Chunk {i}] doc={doc} chunk={cid} | {uu} | pages={pg} | score={c.get('score'):.3f}{txt}")
    return "".join(lines)
...
# ----------------- Agentic RAG main (Answerer-only) -----------------
def agentic_rag(query_original: str) -> Dict[str, Any]:
    ...
    try:
        ...
        iterations_used = 0  # We will force this to 1 in practice
        judge_reports: List[Dict[str, Any]] = []  # kept for shape compatibility; remains empty

        # Single iteration (Answerer-only)
        iterations_used = 1
        log(f"--- Iteration 1/1 ---")
        ...
        log(f"[Step 0] Embedded query in {(time.time()-t0)*1000:.0f} ms")
        log(f"[Step 0] Query used this iteration: {query_original}")

        # Agent 1 & 1b (optional signals, kept for structure)
        t1 = time.time()
        _ = agent1_extract_entities_predicates(query_original)
        _ = agent1b_extract_query_triples(query_original)
        log(f"[Step 1] Entity/Triple extraction in {(time.time()-t1)*1000:.0f} ms")

        # Retrieval: vector search over TextChunk
        t2 = time.time()
        candidates = vector_query_chunks(q_emb, k=TOP_K_CHUNKS)
        log(f"[Step 2] Vector search returned {len(candidates)} candidates in {(time.time()-t2)*1000:.0f} ms")
        ...
        if not candidates:
            ...
            log("[Context preview]:")
            log("".join(top_context.splitlines()[:30]))  # preview at most 30 lines

            # Agent 2: Answer
            t3 = time.time()
            intermediate_answer = agent2_answer(query_original, top_context, guidance=None, output_lang=user_lang)
            log(f"[Step 3] Answer generated in {(time.time()-t3)*1000:.0f} ms")
            final_answer = intermediate_answer

        log("=== Agentic RAG (Answerer-only) summary ===")
        log(f"- Iterations used: {iterations_used}")
        log("=== Final Answer ===")
        log(final_answer or "")
        ...
        return {
            ...
            "judge_reports": judge_reports  # kept for downstream compatibility; empty
        }
    finally:
        if _LOGGER is not None:
            _LOGGER.close()
