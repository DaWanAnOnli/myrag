...
# - Use Agent 2 (Answerer) + Agent 3 (Judge) in an iterative loop
    ...
    try:
        ...
        for it in range(1, MAX_ITERS + 1):
            ...
            if not candidates:
                ...
                # Pass full (albeit empty) context to Judge, with correct argument order
                judge = agent3_judge(query_original, intermediate_answer, context_text, output_lang=user_lang)
            ...
            # Agent 2: Answer
            t3 = time.time()
            intermediate_answer = agent2_answer(query_original, top_context, guidance_prev, output_lang=user_lang)
            log(f"[Step 3] Intermediate answer in {(time.time()-t3)*1000:.0f} ms")

            # Agent 3: Judge (now receives the FULL context, not just headers)
            t4 = time.time()
            judge = agent3_judge(query_original, intermediate_answer, top_context, output_lang=user_lang)
            log(f"[Step 4] Judge in {(time.time()-t4)*1000:.0f} ms")
            judge_reports.append(judge)
        ...
        if judge_reports:
            log(f"- Last judge verdict: sufficient={judge_reports[-1].get('is_sufficient')}")
        log("\n=== Final Answer ===")
        log(final_answer)
