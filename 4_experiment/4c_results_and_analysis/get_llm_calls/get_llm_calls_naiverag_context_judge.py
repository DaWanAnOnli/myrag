# analyze_llm_calls_context_judge.py
"""
Analyzes log files from the Context Judge + Query Modifier RAG.
Tracks LLM calls per query including iterative context evaluation and query refinement.
...
LOG_FOLDER = SCRIPT_DIR / "../../4b_retrieval/4b_ii_naiverag/question_terminal_logs_naive_over_graph/naiverag_5_context_judge_1250"
OUTPUT_FOLDER = SCRIPT_DIR / "llm_calls_analysis_results_naiverag_5_context_judge_1250"
...
def extract_iterations_info(log_path: Path) -> tuple[int, List[Dict[str, bool]]]:
    ...
    try:
        with open(log_path, 'r', encoding='utf-8') as f:
            ...
            judge_sufficient = re.findall(r'\[Judge\] sufficient=(True|False)', content)
            ...
            for i in range(1, num_iterations + 1):
                ...
                if i <= len(judge_sufficient):
                    detail['judge_sufficient'] = judge_sufficient[i-1] == 'True'
                
                # Check if this iteration ended with answer
                if i == num_iterations or (detail['judge_sufficient'] == True):
...
def analyze_log_file(log_path: Path) -> Dict:
    ...
    result = {
        ...
        'context_judge_calls': 0,  # Context Judge per iteration
        'query_modifier_calls': 0,  # Query Modifier when insufficient
        'agent2_calls': 0,  # Final answerer (should be 1)
        ...
        'mode': 'unknown',
        'final_verdict': None  # 'sufficient', 'cap_reached', or None
    }
    ...
    try:
        with open(log_path, 'r', encoding='utf-8') as f:
            ...
            # Detect mode
            if "Agentic RAG (Context Judge + Query Modifier)" in content:
                result['mode'] = 'context-judge-modifier'
            ...
            # Count Context Judge prompts
            judge_matches = re.findall(r'\[Judge\] Prompt:', content)
            result['context_judge_calls'] = len(judge_matches)
            ...
            # Count Agent 2 prompts (final answerer, should be 1)
            ...
            result['total_llm_calls'] = (
                result['context_judge_calls'] + 
                result['query_modifier_calls'] + 
                result['agent2_calls']
            )
            
            # Count judge verdicts
            judge_sufficient = re.findall(r'\[Judge\] sufficient=True', content)
            judge_insufficient = re.findall(r'\[Judge\] sufficient=False', content)
            result['judge_sufficient_count'] = len(judge_sufficient)
            result['judge_insufficient_count'] = len(judge_insufficient)
            
            # Determine final verdict
            if "Judge deemed context sufficient" in content:
                result['final_verdict'] = 'sufficient'
            elif "Reached MAX_ITERATIONS" in content:
...
def save_results(results: List[Dict]):
    ...
    csv_fields = [
        ...
        'total_llm_calls', 'context_judge_calls', 'query_modifier_calls', 'agent2_calls',
        'judge_sufficient_count', 'judge_insufficient_count',
    ...
    with open(iter_txt_path, 'w', encoding='utf-8') as f:
        separator = "=" * 80
        title = "CONTEXT JUDGE ITERATIONS BREAKDOWN - HUMAN READABLE"
        f.write(separator + "\n")
        f.write(title + "\n")
        f.write(separator + "\n")
...
def create_summary(results: List[Dict]) -> str:
    ...
    # Filter by mode
    judge_results = [r for r in results if r['mode'] == 'context-judge-modifier']
    other_results = [r for r in results if r['mode'] != 'context-judge-modifier']
    
    # Overall stats
    total_llm_calls = sum(r['total_llm_calls'] for r in results)
    total_judge = sum(r['context_judge_calls'] for r in results)
    ...
    if judge_results:
        ...
        total_sufficient = sum(r['judge_sufficient_count'] for r in judge_results)
        ...
        total_sufficient = total_insufficient = 0
    ...
    header = """
╔══════════════════════════════════════════════════════════════╗
║    CONTEXT JUDGE + QUERY MODIFIER LLM CALL ANALYSIS         ║
╚══════════════════════════════════════════════════════════════╝
"""
...
│  ├─ Context judge-modifier mode: {len(judge_results)}
│  └─ Other modes: {len(other_results)}
├─ Total LLM calls: {total_llm_calls}
│  ├─ Context judge calls: {total_judge}
...
"""
    ...
    if judge_results:
        iter_section = f"""
🔄 ITERATIVE CONTEXT EVALUATION ANALYSIS ({len(judge_results)} queries):
...
"""
        ...
        iter_section += f"""
⚖️ Context Judge Verdict Analysis:
├─ Total "sufficient" verdicts: {total_sufficient}
...
"""
    ...
    pipeline_doc = """
💡 Pipeline Pattern (Context Judge + Query Modifier):
   └─ For each query:
      For each iteration (up to MAX_ITERATIONS):
         1. Embed current query (1 embedding)
         2. Vector search TextChunk nodes
         3. Context Judge evaluates retrieved context (1 LLM call)
            ├─ If sufficient → Agent 2 answers (1 LLM call) → DONE
            └─ If insufficient → Query Modifier rewrites query (1 LLM call) → next iteration
      At MAX_ITERATIONS cap: skip judge/modifier and answer directly
   └─ Expected LLM calls: N (judges) + (N-1 or fewer) (modifiers) + 1 (answerer)
   └─ Iterative refinement: each iteration uses improved query based on prior feedback
   └─ Agent 1/1b present but calls excluded from LLM count per user request
"""
    
    agent_breakdown = f"""
🔍 Agent Call Breakdown (across all queries):
├─ Context Judge (evaluates context): {total_judge}
├─ Query Modifier (refines query): {total_modifier}
├─ Agent 2 (final answerer): {total_agent2}
...
└─ Context Judge + Query Modifier characteristics:
   ├─ Evaluates context quality before answering
   ├─ Iteratively refines query to improve retrieval
   ├─ Uses naive RAG (vector search) for each iteration
   ├─ No graph traversal or triple matching
   ├─ Judge focuses on context sufficiency vs user query
   ├─ Cost scales with number of iterations needed
   └─ Best for queries requiring precise retrieval targeting
"""
...
"""
    ...
    # First-try sufficient
    first_try = [r for r in judge_results if r.get('num_iterations') == 1 and r.get('judge_sufficient_count') > 0]
    first_try_section = ""
    if first_try:
        first_try_section = f"""
✅ QUERIES WITH SUFFICIENT CONTEXT IN FIRST ITERATION ({len(first_try)} queries):
...
"""
    ...
    notes_doc = """
📝 Notes:
   - Context Judge evaluates if retrieved chunks can answer the query
   - Query Modifier rewrites query when context is insufficient
   - Hard cap on iterations: MAX_ITERATIONS = 4 (configurable)
   - At cap: answer produced regardless of context quality
   - Pattern: retrieve → judge → (answer OR modify → next iteration)
...
def main():
    ...
    separator = "=" * 70
    title = "CONTEXT JUDGE + QUERY MODIFIER LLM CALL ANALYZER"
    subtitle = "(Tracking iterative context evaluation and query refinement)"
