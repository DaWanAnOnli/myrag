...
CSV_FILE = Path("../../../dataset/4_experiment/4c_experiment_results/new/llm_judge_results_20251114-214307_no_4_graphrag_subgoal.csv")

# Layer 1 folder containing the layer 2 experiment folders
LAYER1_FOLDER = Path("../../4b_retrieval/4b_i_lexidkg_graphrag/question_terminal_logs/no_4_subgoal")

# Mapping of folder names to CSV column names
FOLDER_TO_COLUMN = {
    "no_4_lexidkg_2_subgoals_new_fix": "graphrag_2_subgoal_answer score",
    "no_4_lexidkg_3_subgoals_new_fix": "graphrag_3_subgoal_answer score",
    "no_4_lexidkg_4_subgoals_new_fix": "graphrag_4_subgoal_answer score",
    "no_4_lexidkg_5_subgoals_new_fix": "graphrag_5_subgoal_answer score",
}

# Output directory for results
OUTPUT_DIR = Path("results_4_subgoals")
...
def extract_subgoals_from_log(log_path: Path) -> int:
    """
    Extract the number of subgoals used from a log file.
    Uses multiple methods with explicit count preferred.
    """
    try:
        with open(log_path, 'r', encoding='utf-8') as f:
            content = f.read()
        
        # Method 1: Look for explicit count in "subgoals=X" pattern
        # This appears in: "[Agent 0] Decomposition needed? ... | subgoals=2 | 1234.0 ms"
        matches = re.findall(r'\[Agent 0\].*?\|\s*subgoals=(\d+)\s*\|', content)
        if matches:
            # Take the last occurrence (most recent/final)
            return int(matches[-1])
        
        # Method 2: Look for "for X subgoal(s)" in summary
        # This appears in: "- Parallel subgoals time: 1234.0 ms for 2 subgoal(s)"
        matches = re.findall(r'for\s+(\d+)\s+subgoal\(s?\)', content)
        if matches:
            # Take the last occurrence
            return int(matches[-1])
        
        # Method 3: Count "--- Subgoal X/Y" patterns and get Y (total)
        matches = re.findall(r'---\s*Subgoal\s+\d+/(\d+)\s*(?:START|DONE|FAILED)', content)
        if matches:
            # All should have the same total, take the first
            return int(matches[0])
        
        # Method 4: Try to parse the actual list from "[Agent 0] Final subgoal list (used):"
        # Look for the line and try to parse it as a Python literal
        match = re.search(r'\[Agent 0\]\s*Final subgoal list \(used\):\s*(\[.*?\])\s*$', 
                         content, re.MULTILINE)
        if match:
            try:
                list_str = match.group(1)
                # Use ast.literal_eval to safely parse the list
                subgoals_list = ast.literal_eval(list_str)
                if isinstance(subgoals_list, list):
                    return len(subgoals_list)
            except (SyntaxError, ValueError):
                pass
        
        # Method 5: Alternative parsing of the subgoals list
        # Look for "[Agent 0] Subgoals:" followed by a list
        match = re.search(r'\[Agent 0\]\s*Subgoals:\s*(\[.*?\])', content, re.DOTALL)
        if match:
            try:
                list_str = match.group(1)
                # Clean up in case it spans multiple lines
...
def process_experiment_folder(folder_path: Path, column_name: str, 
    ...
    Returns: List of (question_id, subgoals, score) tuples
    """
    results = []
    failed_extractions = []
    ...
    for log_file in log_files:
        ...
        # Extract subgoals from log content
        subgoals = extract_subgoals_from_log(log_file)
        if subgoals == -1:
        ...
        results.append((q_id, subgoals, score))
    
    if failed_extractions:
        print(f"  WARNING: Failed to extract subgoals from {len(failed_extractions)} files:")
...
def analyze_results(results: List[Tuple[int, int, int]], experiment_name: str) -> str:
    ...
    by_subgoals = defaultdict(list)
    for q_id, subgoals, score in results:
        by_subgoals[subgoals].append((q_id, score))
    ...
    lines.append("SUBGOAL DISTRIBUTION:")
    lines.append("-" * 40)
    for subgoals in sorted(by_subgoals.keys()):
        count = len(by_subgoals[subgoals])
        pct = (count / len(results) * 100) if len(results) > 0 else 0
        lines.append(f"  {subgoals} subgoal(s): {count:4d} questions ({pct:5.2f}%)")
    lines.append("")
    ...
    for subgoals in sorted(by_subgoals.keys()):
        questions = by_subgoals[subgoals]
        lines.append("")
        lines.append(f"Questions requiring {subgoals} subgoal(s): {len(questions)} total")
        lines.append("-" * 40)
        
        # Count scores
        subgoal_score_counts = defaultdict(int)
        for _, score in questions:
            if score >= 0:
                subgoal_score_counts[score] += 1
        
        total_subgoal_scored = sum(subgoal_score_counts.values())
        if total_subgoal_scored > 0:
            for score in sorted(subgoal_score_counts.keys()):
                count = subgoal_score_counts[score]
                pct = (count / total_subgoal_scored * 100)
                lines.append(f"  Score {score}: {count:4d} questions ({pct:5.2f}%)")
            
            avg = sum(score * count for score, count in subgoal_score_counts.items()) / total_subgoal_scored
...
def main():
    print("Starting subgoal analysis...")
    ...
    for folder_name, column_name in FOLDER_TO_COLUMN.items():
        ...
        with open(results_file, 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow(['question_id', 'subgoals_required', 'llm_judge_score'])
            for q_id, subgoals, score in sorted(results):
                writer.writerow([q_id, subgoals, score])
    ...
    print("Subgoal analysis complete!")
