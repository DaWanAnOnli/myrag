#!/usr/bin/env python3
"""
Context Judge Iteration Analysis Script
Extracts iteration counts from RAG log files, maps to LLM judge scores,
...
CSV_FILE = Path("../../../dataset/4_experiment/4c_experiment_results/new/llm_judge_results_20251114-223134_no_7_naiverag_context_judge.csv")

# Base directory that contains the 4 experiment folders
LAYER1_FOLDER = Path("../../4b_retrieval/4b_ii_naiverag/question_terminal_logs_naive_over_graph/no_7_context_judge")

# Mapping of folder names to CSV column names
FOLDER_TO_COLUMN = {
    "no_7_naiverag_2_context_judge_1250": "naiverag_2_context_judge_answer score",
    "no_7_naiverag_3_context_judge_1250": "naiverag_3_context_judge_answer score",
    "no_7_naiverag_4_context_judge_1250": "naiverag_4_context_judge_answer score",
    "no_7_naiverag_5_context_judge_1250": "naiverag_5_context_judge_answer score",
}

# Output directory for results
OUTPUT_DIR = Path("context_judge_analysis_results")
...
def extract_iterations_from_log(log_path: Path) -> int:
    """
    Extract the actual number of iterations executed from a log file.
    ...
    try:
        ...
        # Method 1: Look for "- Iterations executed: X" in the summary section
        # This is the most reliable as it's the final count
        match = re.search(r'^-\s+Iterations\s+executed:\s+(\d+)', content, re.MULTILINE)
        if match:
            return int(match.group(1))
        
        # Method 2: Find the highest iteration number from "=== Iteration X/Y ===" patterns
        # This shows all iterations that were started
        matches = re.findall(r'^===\s+Iteration\s+(\d+)/\d+\s+===', content, re.MULTILINE)
        if matches:
            return max(int(m) for m in matches)
        
        # Method 3: Count unique "[Iter X]" prefixes
        matches = re.findall(r'^\[Iter\s+(\d+)\]', content, re.MULTILINE)
        if matches:
            return max(int(m) for m in matches)
        
        # Method 4: Look for individual iteration markers in history
        # "  • Iter X:" in summary
        matches = re.findall(r'^\s+•\s+Iter\s+(\d+):', content, re.MULTILINE)
        if matches:
            return max(int(m) for m in matches)
...
def analyze_results(results: List[Tuple[int, int, int]], experiment_name: str) -> str:
    ...
    for q_id, iterations, score in results:
        by_iterations[iterations].append((q_id, score))
    ...
    for iterations in sorted(by_iterations.keys()):
        count = len(by_iterations[iterations])
        pct = (count / len(results) * 100) if len(results) > 0 else 0
        lines.append(f"  {iterations} iteration(s): {count:4d} questions ({pct:5.2f}%)")
    lines.append("")
    ...
    for iterations in sorted(by_iterations.keys()):
        questions = by_iterations[iterations]
        lines.append("")
        lines.append(f"Questions requiring {iterations} iteration(s): {len(questions)} total")
        lines.append("-" * 40)
...
def main():
    print("Starting Context Judge Iteration analysis...")
    ...
    print("Context Judge Iteration analysis complete!")
