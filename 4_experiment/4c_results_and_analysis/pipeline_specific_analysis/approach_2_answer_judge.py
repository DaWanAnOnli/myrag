#!/usr/bin/env python3
"""
Multi-Agent Answer Judge Iteration Analysis Script
Extracts iteration counts from multi-agent RAG log files, maps to LLM judge scores,
...
CSV_FILE = Path("../../../dataset/4_experiment/4c_experiment_results/new/llm_judge_results_20251121-202846_no_17_answer_judge_2nd.csv")

# Layer 1 folder containing the layer 2 experiment folders
LAYER1_FOLDER = Path("../../4b_retrieval/4b_iv_multi_agents/question_terminal_logs_multi_agent/no_17_answer_judge_2")

# Mapping of folder names to CSV column names
FOLDER_TO_COLUMN = {
    "no_17_approach_2_both_2_answer_judge_2nd": "approach_2_2_answer_judge_answer score",
    "no_17_approach_2_both_3_answer_judge_2nd": "approach_2_3_answer_judge_answer score",
    "no_17_approach_2_both_4_answer_judge_2nd": "approach_2_4_answer_judge_answer score",
    "no_17_approach_2_both_5_answer_judge_2nd": "approach_2_5_answer_judge_answer score",
}

# Output directory for results
OUTPUT_DIR = Path("results_17_answer_judge")
...
def extract_iterations_from_log(log_path: Path) -> int:
    ...
    try:
        ...
        # Method 1: Look for "- Total iterations used: X" in the summary section
        # This is the most reliable as it's the final count
        match = re.search(r'^.*-\s+Total\s+iterations\s+used:\s+(\d+)', content, re.MULTILINE)
        if match:
            return int(match.group(1))
        
        # Method 2: Find the highest iteration number from "========== [Iter X/Y] ==========" patterns
        # Multi-agent uses equals signs with spaces
        matches = re.findall(r'=+\s+\[Iter\s+(\d+)/\d+\]\s+=+', content)
        if matches:
            return max(int(m) for m in matches)
        
        # Method 3: Count unique "[Iter X]" prefixes (throughout the log)
        matches = re.findall(r'^\[.*?\]\s+\[.*?\]\s+\[.*?\]\s+\[Iter\s+(\d+)\]', content, re.MULTILINE)
        if matches:
            return max(int(m) for m in matches)
        
        # Method 4: Less specific [Iter X] pattern
        matches = re.findall(r'\[Iter\s+(\d+)\]', content)
        if matches:
            return max(int(m) for m in matches)
...
def main():
    print("Starting Multi-Agent Iteration analysis...")
    ...
    print("Multi-Agent Iteration analysis complete!")
