...
CSV_FILE = Path("../../../dataset/4_experiment/4c_experiment_results/new/Individual base multi-agent - approach_2_query_decomposer.csv")

# Layer 1 folder containing the layer 2 experiment folders
LAYER1_FOLDER = Path("../../4b_retrieval/4b_iv_multi_agents/question_terminal_logs_multi_agent/no_16_approach_2_query_decomposer")



# Mapping of folder name to CSV column name
FOLDER_TO_COLUMN = {
    "no_16_approach_2_query_decomposer_new": "approach_2_query_decomposer_answer score",
}

# Output directory for results
OUTPUT_DIR = Path("results_no_16_query_decomposer")
...
def extract_task_pattern(content: str) -> Optional[str]:
    ...
    The actual log format from the script is:
    [Decomposer] NaiveRAG task: role=primary, aspect=..., query=...
    [Decomposer] GraphRAG task: role=support, aspect=..., query=...
    
    Returns normalized pattern like "1G_primary, 1N_support"
    """
    # Pattern 1: Match the actual log format
    # [Decomposer] <Pipeline> task: role=<role>
    task_matches = re.findall(
        r'\[Decomposer\]\s+(NaiveRAG|GraphRAG)\s+task:\s+role\s*=\s*(\w+)',
        content,
        re.IGNORECASE
    )
    ...
    if task_matches:
        ...
        graphrag_primary = sum(1 for p, r in task_matches 
                              if p.lower() == "graphrag" and r.lower() == "primary")
        graphrag_support = sum(1 for p, r in task_matches 
                              if p.lower() == "graphrag" and r.lower() == "support")
        naiverag_primary = sum(1 for p, r in task_matches 
                              if p.lower() == "naiverag" and r.lower() == "primary")
        naiverag_support = sum(1 for p, r in task_matches 
                              if p.lower() == "naiverag" and r.lower() == "support")
    ...
    # Pattern 2: Try alternative format (from decompose_query output)
    # Look for logged tasks in different format
    alt_matches = re.findall(
        r'\[Decomposer\]\s+(\w+)\s+task.*?role\s*=\s*(\w+)',
        content,
        re.IGNORECASE | re.DOTALL
    )
    
    if alt_matches:
        graphrag_primary = sum(1 for p, r in alt_matches 
                              if 'graph' in p.lower() and r.lower() == "primary")
        graphrag_support = sum(1 for p, r in alt_matches 
                              if 'graph' in p.lower() and r.lower() == "support")
        naiverag_primary = sum(1 for p, r in alt_matches 
                              if 'naive' in p.lower() and r.lower() == "primary")
        naiverag_support = sum(1 for p, r in alt_matches 
                              if 'naive' in p.lower() and r.lower() == "support")
    ...
    # Pattern 3: Look in the decomposer plan output in the final result
    plan_match = re.search(
        r'"decomposer_plan":\s*\{[^}]*"subqueries_naive":\s*\[([^\]]*)\][^}]*"subqueries_graphrag":\s*\[([^\]]*)\]',
        content,
        re.IGNORECASE | re.DOTALL
    )
    
    if plan_match:
        naive_subqs_str = plan_match.group(1)
        graphrag_subqs_str = plan_match.group(2)
        
        # Count non-empty queries
        naive_count = len([s for s in re.findall(r'"([^"]+)"', naive_subqs_str) if s.strip()])
        graphrag_count = len([s for s in re.findall(r'"([^"]+)"', graphrag_subqs_str) if s.strip()])
        
        parts = []
        if graphrag_count > 0:
            parts.append(f"{graphrag_count}G_primary")
        if naive_count > 0:
            parts.append(f"{naive_count}N_primary")
...
def extract_aggregator_decision_from_log(log_path: Path) -> str:
    ...
    try:
        ...
        # Method 5: Look in the final result JSON
        agg_match = re.search(
            r'"aggregator_decision":\s*\{[^}]*"decision":\s*"(choose_graphrag|choose_naiverag|merge)"',
            content,
            re.IGNORECASE
        )
        if agg_match:
            return agg_match.group(1).strip().lower()
...
def process_experiment_folder(folder_path: Path, column_name: str, 
    ...
    if log_files:
        ...
        try:
            with open(sample_file, 'r', encoding='utf-8') as f:
                content = f.read()
                # Show decomposer-related log lines
                decomposer_lines = [line for line in content.split('\n') if '[Decomposer]' in line]
                if decomposer_lines:
                    print("  Sample decomposer log lines:")
                    for line in decomposer_lines[:5]:
                        print(f"    {line[:150]}")
                else:
                    print("  WARNING: No [Decomposer] lines found in sample file!")
                
                # Show aggregator-related log lines
                aggregator_lines = [line for line in content.split('\n') if '[Aggregator]' in line]
                if aggregator_lines:
                    print("  Sample aggregator log lines:")
                    for line in aggregator_lines[:3]:
                        print(f"    {line[:150]}")
                else:
                    print("  WARNING: No [Aggregator] lines found in sample file!")
        except Exception as e:
    ...
    if failed_decomposer:
        ...
        if failed_decomposer:
            q_id, fname = failed_decomposer[0]
            ...
            try:
                ...
                with open(log_path, 'r', encoding='utf-8') as f:
                    content = f.read()
                    decomposer_lines = [line for line in content.split('\n') if 'decomposer' in line.lower()][:10]
                    if decomposer_lines:
                        print("    Decomposer-related lines from this file:")
                        for line in decomposer_lines:
                            print(f"      {line[:150]}")
            except Exception as e:
