#!/usr/bin/env python3
"""
Subgoal Analysis Script
Extracts subgoal counts from RAG log files, maps to LLM judge scores,
...
CSV_FILE = Path("../../../dataset/4_experiment/4c_experiment_results/new/llm_judge_results_20251114-212649_no_3_naiverag_subgoal.csv")

# Layer 1 folder containing the layer 2 experiment folders
LAYER1_FOLDER = Path("../../4b_retrieval/4b_ii_naiverag/question_terminal_logs_naive_over_graph/no_3_subgoal")


# Mapping of folder names to CSV column names
FOLDER_TO_COLUMN = {
    "no_3_naiverag_2_subgoals_1250": "naiverag_2_subgoal_answer score",
    "no_3_naiverag_3_subgoals_1250": "naiverag_3_subgoal_answer score",
    "no_3_naiverag_4_subgoals_1250": "naiverag_4_subgoal_answer score",
    "no_3_naiverag_5_subgoals_1250": "naiverag_5_subgoal_answer score",
}

# Output directory for results
OUTPUT_DIR = Path("results_3_subgoals")
...
def extract_subgoals_from_log(log_path: Path) -> int:
    """
    Extract the actual number of subgoals used from a log file.
    ...
    try:
        ...
        # Method 1: Look for "[SubgoalGen] Produced X subgoal(s):"
        # This is the most reliable as it's logged right after generation
        match = re.search(r'\[SubgoalGen\]\s+Produced\s+(\d+)\s+subgoal\(s?\):', content)
        if match:
            return int(match.group(1))
        
        # Method 2: Look for "- Subgoals: X" in the summary section
        match = re.search(r'^-\s+Subgoals:\s+(\d+)', content, re.MULTILINE)
        if match:
            return int(match.group(1))
        
        # Method 3: Count the logged subgoal lines "  - SG\d+: ..."
        # These are logged right after "Produced X subgoal(s):"
        matches = re.findall(r'^\s+-\s+SG\d+:', content, re.MULTILINE)
        if matches:
            return len(matches)
        
        # Method 4: Look for "• SGX:" in summary
        matches = re.findall(r'^\s+•\s+SG\d+:', content, re.MULTILINE)
        if matches:
            return len(matches)
