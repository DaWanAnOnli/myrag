...
CSV_FILE = Path("../../../dataset/4_experiment/4c_experiment_results/new/llm_judge_results_20251114-221530_no_8_graphrag_context_judge.csv")

# Layer 1 folder containing the layer 2 experiment folders
LAYER1_FOLDER = Path("../../4b_retrieval/4b_i_lexidkg_graphrag/question_terminal_logs/no_8_context_judge")

# Mapping of folder names to CSV column names
FOLDER_TO_COLUMN = {
    "no_8_lexidkg_2_context_judge_5_hop_1250": "graphrag_2_context_judge_answer score",
    "no_8_lexidkg_3_context_judge_5_hop_1250": "graphrag_3_context_judge_answer score",
    "no_8_lexidkg_4_context_judge_5_hop_1250": "graphrag_4_context_judge_answer score",
    "no_8_lexidkg_5_context_judge_5_hop_1250": "graphrag_5_context_judge_answer score",
}

# Output directory for results
OUTPUT_DIR = Path("results_8_context_judge")
...
def extract_iterations_from_log(log_path: Path) -> int:
    ...
    Looks for lines like: "- Iterations used: 2 (cap=4)"
    or "- Iterations used: 1"
    """
    try:
        ...
        match = re.search(r'Iterations used:\s*(\d+)', content)
        if match:
            return int(match.group(1))
        
        # Alternative: count how many "--- Iteration X/Y" lines appear
        iteration_matches = re.findall(r'--- Iteration (\d+)/(\d+)', content)
...
def main():
    ...
    print("Context Judge analysis complete!")
