#!/usr/bin/env python3
# run_doc_index.py
# Runner to launch many concurrent doc_index_pipeline.py processes,
...
if IS_SAMPLE:
    BATCHES_ROOT = (BASE_DIR / "../../dataset/samples/3_indexing/3f1_langchain_whole_document_batches").resolve()
    JSON_OUTPUT_DIR = (BASE_DIR / "../../dataset/samples/3_indexing/3f_llm_json_outputs_whole_document").resolve()
else:
    BATCHES_ROOT = (BASE_DIR / "../../dataset/3_indexing/3f1_langchain_whole_document_batches").resolve()
    JSON_OUTPUT_DIR = (BASE_DIR / "../../dataset/3_indexing/3f_llm_json_outputs_whole_document").resolve()

REPO_ROOT = BASE_DIR.parent.parent  # parent directory of this script

DOC_PIPELINE_PATH = (BASE_DIR / "whole_document_pipeline.py").resolve()
TMP_SCRIPT_PREFIX = "_whole_document_run_"
...
def confirm_json_output_dir(skip_confirm: bool = False):
    # Not required for indexing; kept for parity and to avoid path surprises
...
def read_doc_pipeline_source() -> str:
    if not DOC_PIPELINE_PATH.exists():
        raise FileNotFoundError(f"doc_index_pipeline.py not found at {DOC_PIPELINE_PATH}")
    return DOC_PIPELINE_PATH.read_text(encoding="utf-8")

def write_modified_script(i: int, lang_dir: Path, source: str) -> Path:
    """
    Creates a sibling copy of doc_index_pipeline.py named _doc_index_run_{i}.py
    with:
    ...
    if n_api == 0:
        raise RuntimeError("Failed to patch GOOGLE_API_KEY line in doc_index_pipeline.py")
    ...
    if n_dir == 0:
        raise RuntimeError("Failed to patch DEFAULT_LANGCHAIN_DIR in doc_index_pipeline.py")

    out_path = DOC_PIPELINE_PATH.parent / f"{TMP_SCRIPT_PREFIX}{i}.py"
...
def launch_process(run_id: int, script_path: Path, json_out_dir: Path, key_id: int):
    ...
    env["JSON_OUTPUT_DIR"] = str(json_out_dir)  # unused; kept for parity

    LOGS_DIR.mkdir(parents=True, exist_ok=True)
    ts = time.strftime("%Y%m%d-%H%M%S")
    log_path = LOGS_DIR / f"doc_run_{run_id:02d}_key{key_id}_{ts}.log"
    ...
    proc = subprocess.Popen(
        cmd,
        cwd=str(DOC_PIPELINE_PATH.parent),
...
def main():
    ...
    source = read_doc_pipeline_source()
    script_paths = []
    for i in nums:
