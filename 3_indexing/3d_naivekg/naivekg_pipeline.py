...
SYSTEM_HINT = """
You extract entity–relationship facts from text.

Output JSON must be:
{
  "triples": [
    {
      "subject": { "text": "entity name", "canonical_id": "optional" },
      "predicate": "relationship phrase (short; snake_case preferred but free text OK)",
      "object":  { "text": "entity name", "canonical_id": "optional" },
      "evidence": {
        "quote": "short supporting span from the chunk",
        "char_start": <starting char from chunk that contains evidence>,
        "char_end": <ending char from chunk that contains evidence>,
        "article_ref": "e.g., 'Pasal 5 (1)'"
      },
...
Guidelines:
- Do not invent facts; use only what appears in the text chunk.
- Keep entity texts as found.
- If the relation is unclear, use a neutral predicate like "relates_to".
"""
...
Extract entity–relationship triples from the text. Return only JSON.
...
def save_error_json_output(error_stub: Dict[str, Any], context: Dict[str, Any], attempt_id: int, err_type: str) -> Path:
    ...
    try:
        # Ensure stub is not empty (it won't be; includes error info)
        with open(out_path, "w", encoding="utf-8") as f:
        ...
        # Fallback: do not raise further to avoid masking original error
        return out_path
...
def run_llm_json(prompt: str, schema: dict, context: Dict[str, Any]) -> Tuple[Dict[str, Any], float, int, Optional[str]]:
    ...
    # Enriched initial log with chunk_ids/docs/uu
    items_metas: List[Dict[str, Any]] = context.get("items_metas", [])
    ...
    try:
        ...
        # Raise typed error carrying json_path
        if is_rate_limit_error(e):
...
def extract_triples_from_chunk(chunk_text: str, meta: Dict[str, Any], prompt_override: Optional[str] = None) -> Tuple[List[Dict[str, Any]], float, int, Optional[str]]:
    ...
    for t in data.get("triples", []):
        ...
        # Normalize predicate to lower_snake_case for storage and for relationship label
        pred = normalize_predicate(pred_raw)

        # Force simple ER node typing
        s_type = "Entity"
        o_type = "Entity"

        # Stable keys per entity
...
def extract_triples_from_chunks_batch(items: List[Dict[str, Any]], prompt: str) -> Tuple[Dict[str, List[Dict[str, Any]]], float, int, Optional[str]]:
    ...
    data, gemini_duration, attempt_id, json_path = run_llm_json(
        prompt, BATCH_TRIPLE_SCHEMA,   # batch schema expected to allow simple ER triples
        ...
        while True:
            ...
            try:
                ...
                for it in items:
                    ...
                    # Assign LLM time to leader only; others share (0.00s)
                ...
                if per_chunk_stats:
                    for it in items_batch:
                        ...
                        if stats:
                            ...
                            # LLM time display: mark shared if zero
