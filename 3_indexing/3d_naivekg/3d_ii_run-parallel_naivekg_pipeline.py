...
if IS_SAMPLE:
    ...
    JSON_OUTPUT_DIR = (BASE_DIR / "../../dataset/samples/3_indexing/3e_llm_json_outputs_naivekg").resolve()
else:
    BATCHES_ROOT = (BASE_DIR / "../../dataset/3_indexing/3b_langchain_batches").resolve()
    JSON_OUTPUT_DIR = (BASE_DIR / "../../dataset/3_indexing/3e_llm_json_outputs_naivekg").resolve()


REPO_ROOT = BASE_DIR.parent.parent  # parent directory of this script
KG_PIPELINE_PATH = (BASE_DIR / "naivekg_pipeline.py").resolve()
TMP_SCRIPT_PREFIX = "naivekg_pipeline_run_"
...
def read_kg_pipeline_source() -> str:
    if not KG_PIPELINE_PATH.exists():
        raise FileNotFoundError(f"naive-kg-pipeline.py not found at {KG_PIPELINE_PATH}")
    return KG_PIPELINE_PATH.read_text(encoding="utf-8")

def write_modified_script(i: int, lang_dir: Path, source: str) -> Path:
    """
    Creates a sibling copy of naive-kg-pipeline.py named _kg_pipeline_run_{i}.py
    with:
